...
class PrintStatement(ASTNode):
    ...
    def execute(self):
        # Create list to temporarily hold expressions from source code
        output = []
        # Iterate through every expression within the source code's print statement
        for expr in self.expressions.evaluate():
            if isinstance(expr, str): # Check if current expression is a string
                # Strip quotation marks from string
                if expr.startswith('"') and expr.endswith('"'):
                    expr = expr[1:-1]
            output.append(str(expr))  # Append current expr to "output" list as a string
        # Join together everything in output list to be printed, separated by a space
        print(" ".join(output))
